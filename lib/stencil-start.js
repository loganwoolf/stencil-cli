...
class StencilStart {
    ...
    async startBrowserSync(cliOptions, browserSyncPort) {
        ...
        // Handle keyboard shortcuts while the dev server is running:
        // 't' opens the theme preview, 'm' opens /manage, 'r' reloads,
        // 'q' or Ctrl+C quits the server.
        // Borrowed in part from https://github.com/remy/nodemon
        process.stdin.resume();
        if (typeof process.stdin.setRawMode === 'function') {
            process.stdin.setRawMode(true);
        }
        process.stdin.setEncoding('utf8');
        // Safety net: if the parent process exits for any reason (SIGINT after
        // a forwarded signal, SIGTERM from outside, normal exit, uncaught throw),
        // tear down the webpack-dev-server fork so it doesn't orphan. This
        // listener is synchronous only — no process.exit() inside.
        const shutdownBuildWorker = () => {
            if (this._buildConfigManager?.stopWorker) {
                this._buildConfigManager.stopWorker('SIGTERM');
            }
        };
        process.on('exit', shutdownBuildWorker);
        process.stdin.on('data', (key) => {
            ...
            if (key === 't') {
                this._openUrl(`http://localhost:${browserSyncPort}`);
                return;
            }
            if (key === 'm') {
                this._openUrl(`http://localhost:${browserSyncPort}/manage`);
                return;
            }
            if (key === 'r') {
                this._browserSync.reload();
        ...
        this._logger.log(this.getKeyboardShortcutsInfo());
    ...
    /**
     * Displays the keyboard shortcuts available while the dev server is running.
     * @returns {string}
     */
    getKeyboardShortcutsInfo() {
        let info = '\n';
        info += '-----------------Keyboard Shortcuts-------------\n'.gray;
        info += `  ${'t'.cyan} - open the theme preview in your browser\n`;
        info += `  ${'m'.cyan} - open the manage app (${'/manage'.cyan})\n`;
        info += `  ${'r'.cyan} - reload all connected browsers\n`;
        info += `  ${'q'.cyan} - stop the dev server (or Ctrl+C)\n`;
        info += '-------------------------------------------------\n'.gray;
        return info;
    }
}
