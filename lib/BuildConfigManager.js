...
class BuildConfigManager {
    ...
    initWorker() {
        ...
        if (this._fs.existsSync(buildConfigPath)) {
            this._worker = fork(buildConfigPath, [], {
                cwd: this._workDir,
                detached: true,
            });
    ...
    stopWorker(signal = 'SIGTERM') {
        ...
        // detached: true on init forks the worker into its own process group;
        // sending the signal to -pid cascades to every child process the worker
        // spawned (e.g. fork-ts-checker-webpack-plugin descendants) instead of
        // orphaning them when the parent stencil-cli exits.
        try {
            process.kill(-this._worker.pid, signal);
        } catch (e) {
